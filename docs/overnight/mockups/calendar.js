/* ══════════════════════════════════════════════════════════════════
   ...
   Inert. Loaded only by the mockup pages in this folder and by the
   generated review Artifact; the app never sees it. It renders REAL
   captured ESPN data (data.js) in the app's real design language so
   the two layouts can be compared on a phone without touching any
   shipped code.

   Exposes ONE entry point:

       makeCalendar(rootElement, mode)

   `mode` is the fork Dylan is deciding:
   ...
   It is a factory rather than a script with module-level state because
   the review Artifact puts BOTH modes on one page side by side, and
   two instances sharing a `currentMonth` would page each other.

   Style follows the app: var + function declarations, and DOM built
   with createElement/textContent only (no innerHTML anywhere).
   ══════════════════════════════════════════════════════════════════ */

/* "Today" is PINNED for the mockups. The captured data runs Jul-Oct
   2026 and the real today (Jul 26) sits in a dead week between the
   World Cup final and the NFL opener, so a mockup anchored there would
   show an empty grid and say nothing about the design. Pinning to an
   NFL Sunday shows the today marker under real load. The app itself
   always uses the actual date. */
var MOCK_TODAY = "2026-09-13";
var START_MONTH = "2026-09";

/* Dylan is in PT and the app labels every kickoff "PT", so the mockups
   format and group in PT explicitly. That keeps the screenshots and
...
/* ── Shared helpers (mirrors of the app's) ──────────────────────── */
...
/* ISO instant → "YYYY-MM-DD" in PT. Formatting in the target zone
   rather than doing date maths is what keeps a 00:20Z kickoff (Sunday
...
/* "YYYY-MM-DD" → Date at local noon. Noon, not midnight, so a DST
   shift can never round the date back to the previous day. */
...
/* Same rule as the app's seamColor(): reject a kit colour too light to
   read against a white card (white kits like England's) and fall back
   to the alternate, then to the sport pair. */
...
/* All games in a captured month's padded range, keyed by PT day. */
...
/* Games for an arbitrary day, across every captured month. Needed
   because mode "both" lets the 7-day list run past the end of the
...
function buildCard(g) {
    ...
    if (isPost && g.sc) {
        ...
        /* Modern Intl separates the meridiem with U+202F (narrow
           no-break space), not a plain space, so splitting on a plain
           space alone silently yields one token and drops "AM PT". */
    ...
    var av = el("span", "gc-avail " + (g.av || "can_watch").replace(/_/g, "-"));
...
function makeCalendar(root, mode) {
    var currentMonth = START_MONTH;
    var selectedDate = MOCK_TODAY;
    var windowStart  = MOCK_TODAY;   /* mode "both": first day of the list */
    ...
    function buildMonthNav() {
        ...
        nav.appendChild(el("div", "month-nav-label",
            dayDate(currentMonth + "-15")
    ...
    function buildGrid() {
        ...
        monthDays(currentMonth).forEach(function(ymd) {
            ...
            /* WHICH SPORTS + HOW MANY — one dot per distinct sport,
               then the total when the day holds more than one game.
               The obvious version (one dot per game, capped at n) was
               built first and thrown away: on Sun Sep 13 it drew the
               three earliest games only, all NFL, so the Premier League
               fixture that same morning vanished from the month view
               entirely. A glance layer that hides a whole sport is
               worse than no glance layer. Deduping also ended the
               overflow problem for good — two dots and a two-digit
               number is ~30px inside a 49px cell, where four dots plus
               "+10" was clipping mid-glyph. */
            ...
            sports.forEach(function(sp) {
                ...
                /* Dim a sport only when every one of its games that day
                   is finished, so a half-played day still reads live. */
    ...
    function buildLegend() {
        ...
        /* The number in a cell is not self-explanatory the first time
           you see it, and it is the busiest signal on the grid. */
        var count = el("span");
    ...
    function pickDay(ymd) {
        ...
        if (mode === "both") {
            var first = root.querySelector(".mobile-day");
            if (first) first.scrollIntoView({ block: "start", behavior: "smooth" });
        }
    }

    /* Mode "replace": the selected-day panel under the grid. */
    function buildDayPanel() {
        var panel = el("div", "day-panel");
        var dg = gamesForDay(selectedDate);

        var head = el("div", "day-panel-head");
        head.appendChild(el("span", "day-panel-date",
            dayDate(selectedDate).toLocaleDateString("en-US",
                { weekday: "long", month: "long", day: "numeric" })));
    ...
    function buildRollingList() {
        ...
        /* The feed needs a name once a grid sits above it, otherwise
           the first day header reads as part of the grid. */
    ...
    function render() {
        ...
        root.appendChild(mode === "replace" ? buildDayPanel() : buildRollingList());
    }
