...
- **Standings reset to zeros between seasons** and then sort ALPHABETICALLY. Probed Jul-22: PL = Bournemouth 1st / Arsenal 2nd / Man City 15th, all on 0 pts; every NFL team at `rank 0, 0-0-0`. Any feature that prints a rank must gate on games-played (`app/espn.py:games_played`) or it will state a standing that doesn't exist. `fetch_standings` sets **`preseason: True`** on a league whose every row is at 0 games, and `get_title_races` carries it onto each race — the four render sites (Home mini-tables, Home race card, Tables league sections, Tables race widget) all honour it
